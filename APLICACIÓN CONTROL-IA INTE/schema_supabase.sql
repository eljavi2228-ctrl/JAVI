...
create table if not exists public.subgrupos (
  ...
  analista text not null default 'Sin registrar',
...
alter table public.subgrupos
  ...
  alter column analista set default 'Sin registrar',
  alter column lote set default 'Sin lote';
...
create table if not exists public.datos_importados (
  id uuid primary key default gen_random_uuid(),
  nombre_archivo text not null default 'CSV pegado',
  hoja text,
  tipo_carga text not null default 'Libre',
...
create index if not exists idx_datos_importados_created on public.datos_importados(created_at);
...
alter table public.datos_importados enable row level security;
...
drop policy if exists "authenticated read datos importados" on public.datos_importados;
create policy "authenticated read datos importados"
on public.datos_importados for select to anon, authenticated using (true);

drop policy if exists "authenticated write datos importados" on public.datos_importados;
create policy "authenticated write datos importados"
on public.datos_importados for all to anon, authenticated using (true) with check (true);
